...
public class UITest implements Screen {
    ...
    public UITest(MyGdxGame ref) {
        this.game = ref;

        skin = new Skin(Gdx.files.internal("data/ui/uiskin.json"));
        texture1 = new Texture(Gdx.files.internal("data/img/badlogicsmall.jpg"));
        texture2 = new Texture(Gdx.files.internal("data/img/badlogic.jpg"));
        ...
        final TextButton exit = new TextButton("X", skin);
        exit.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                window.remove();
            }
        });
        window.getButtonTable().add(exit).height(window.getPadTop());
